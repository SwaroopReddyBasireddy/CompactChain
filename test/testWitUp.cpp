...
namespace testWitUp {
    ...
    void test(size_t setSize, string index) {
        ...
        futures.push_back(threadPool.enqueue<void>([&, itr]() {
            RSAAccumulator::witnessesForSet(rep_txo, rsaKey, Wcc, threadPool);
        }));
        itr++;
        futures.push_back(threadPool.enqueue<void>([&, itr]() {
		...
		int step = 200;
        int len = 5;
        vector<double> time_boneh, time_update_boneh, time_proposed, time_update_proposed, time_minichain, time_update_minichain;
        ...
        for(int k=0;k<len;k++){
            ...
            cout << "\nTime consumed to update tx witness in Boneh " << time_update_boneh[k] << endl; 
			if(!(b1 && b2)) {
            ...
            //x_k = elements[2500];
            std::vector<flint::BigInt> reps_e(size);
            ...
            time_update_minichain.push_back(updateEnd - updateStart);
            cout << "\nTime consumed to update tx witness in Minichain " << time_update_minichain[k] << endl;
            ...
            std::vector<flint::BigInt> reps_e_txo(size), reps_e_stxo(size);
            ...
            time_update_proposed.push_back(updateEnd - updateStart);
            cout << "\nTime consumed to update tx witness in CompactChain " << time_update_proposed[k] << endl; 
